...
function FIHC_off!(individual::Individual)
    # flat version
...
function run_new_individual!(p3::Population_Pyramid)
    println("best individual fitness: ", get_fitness!(p3.best_individual))
    ...
    while i <= length(p3.population)
        ...
        old_fitness = get_fitness!(new_individual)
        optimal_mixing_bottom_to_top!(new_individual, p3.population[i].individuals)
        new_fitness = get_fitness!(new_individual)

        if get_fitness!(p3.best_individual) < new_fitness
            p3.best_individual = copy_individual(new_individual)
        ...
        if new_fitness > old_fitness
            ...
            if i == length(p3.population)
                ...
                push!(p3.population, Level(Vector{Individual}(undef, 0), p3.population[i].env_wrapper))
            end
        else
