...
mutable struct EncoderOutputES_Struct <: AbstractOptimizerModule.AbstractOptimizer
...
function AbstractOptimizerModule.get_optimizer(::Val{:EncoderOutputES})
    return EncoderOutputES_Struct
end

function EncoderOutputES_Struct(;
...
function AbstractOptimizerModule.run!(eoes::EncoderOutputES_Struct; max_generations::Int, max_evaluations::Int, log::Bool, visualize_each_n_epochs::Int=0) :: DataFrames.DataFrame

    EnvironmentWrapper.set_verbose!(eoes.current_env_wrapper, log)
    es = eoes.es_type(
        zeros(Float32, EnvironmentWrapper.get_latent_size(eoes.current_env_wrapper), EnvironmentWrapper.get_action_size(eoes.current_env_wrapper))
        ;
        eoes.es_kwargs...
    )
    best_fitness = -Inf
    ...
    for generation in 1:max_generations
        statistics = Environment.get_statistics(eoes.run_statistics)
        ...
        # for i in 0:length(solutions)
            genes_local = i == 0 ? means : solutions[i]
            ind_local = Individuals.Individual(eoes.current_env_wrapper, eoes.individual_config; genes=genes_local, verbose=eoes.verbose)
            if i == 0
        ...
        if visualize_each_n_epochs > 0 && generation % visualize_each_n_epochs == 0
            Environment.visualize!(eoes.visualization_env, Individuals.get_nn(individuals[1]))
        end
